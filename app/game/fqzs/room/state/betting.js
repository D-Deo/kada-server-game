...
class BettingState extends Super {
    constructor(room) {
        super(room, fqzscons.RoomState.BETTING(), fqzscons.RoomStateInterval.BETTING());
        this.betTimer = null;
    }
    ...
    action(seat, action, next) {
        if (action.name === cons.RoomAction.PLAYER_BID()) {
            ...
            if (!utils.isNumber(area, 0, fqzscons.RoomAreaMulti.length - 1) ||
            ...
            if (!utils.isArray(chips, fqzscons.RoomAreaMulti.length, fqzscons.RoomAreaMulti.length)) {
            ...
            // if (this.room.getAttr('bankerCount') > 0) {
