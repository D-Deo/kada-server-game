...
class Seat extends Super {
    ...
    bet(bullet_score, fish_score) {
        ...
        let payRate = payTotal ? ((userJackpot + payTotal) / payTotal * 100) : (userJackpot / 100);
        if (payRate > 100 && payRate > winGoldRate + 100) {
            caught = false;
        }
        if (payRate < 100 && (100 - payRate) > loseGoldRate) {
            caught = true;
        }
        if (userJackpot > 0 && userJackpot > winGold) {
            caught = false;
        }
        if (userJackpot < 0 && -userJackpot > loseGold) {
            caught = true;
        }
